...
import (
	"fmt"
	"nm.go/go-server/library/scheduler"
	"os"
	"sync"
	"time"
)
...
func RunLogger(runningScheduler *scheduler.Handler, appName string, loc *time.Location, showConsoleLog bool) {
	...
	curLogger.loc = loc
	curLogger.showConsoleLog = showConsoleLog
...
func CurrentFileName() string {
	...
	return fmt.Sprintf("%s_%04d-%02d-%02d.log", curLogger.appName, now.Year(), now.Month(), now.Day())
}
